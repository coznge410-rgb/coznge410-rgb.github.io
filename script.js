...
async function loadVisitedPlacesFromGeoServer(map, config) {
    ...
    let url = `${config.baseUrl}/wfs?service=WFS&version=1.0.0&request=GetFeature&typeName=${encodeURIComponent(config.workspace + ':' + config.layer)}&outputFormat=application/json&srsName=${encodeURIComponent(config.srsName)}`;
    if (config.filterMemberName) {
        const cql = `member='${config.filterMemberName.replace(/'/g, "\\'")}'`;
